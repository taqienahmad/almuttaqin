...
async def get_user_by_nis(db: AsyncSession, nis: str) -> User | None:
    result = await db.execute(select(User).where(User.nis == nis))
    return result.scalar_one_or_none()


async def get_user_by_nip(db: AsyncSession, nip: str) -> User | None:
    result = await db.execute(select(User).where(User.nip == nip))
    return result.scalar_one_or_none()
